...
import {
  provider as validateProvider,
  suggestionsShow as validateSuggestions
} from "./validate";
import {
  create as createElement,
  PADDING_CHARACTER
} from "./elements/highlight";
...
export default class ProvidersHighlight {
  ...
  async trigger(textEditor: TextEditor): Promise<Array<HighlightItem>> {
    ...
    const visibleRange = Range.fromObject([
      textEditor.bufferPositionForScreenPosition([
        textEditor.getFirstVisibleScreenRow(),
        0
      ]),
      textEditor.bufferPositionForScreenPosition([
        textEditor.getLastVisibleScreenRow(),
        0
      ])
    ]);
  ...
  paint(textEditor: TextEditor, intentions: Array<HighlightItem>): () => void {
    ...
    for (const intention of (intentions: Array<HighlightItem>)) {
      ...
      marker.onDidChange(function({
        ...
        element.textContent = PADDING_CHARACTER.repeat(
          textEditor.getTextInBufferRange([start, end]).length
        );
      });
      markers.push(marker);
    }
