...
describe("Intentions list element", function() {
  it("has a complete working lifecycle", function() {
    ...
    const suggestions = [
      createSuggestion(
        "Suggestion 1",
        jasmine.createSpy("suggestion.selected.0"),
        "someClass",
        "someIcon"
      ),
      createSuggestion(
        "Suggestion 2",
        jasmine.createSpy("suggestion.selected.1")
      ),
      createSuggestion(
        "Suggestion 3",
        jasmine.createSpy("suggestion.selected.2"),
        "anotherClass"
      )
    ];
    ...
    expect(rendered.refs.list.children[0].children[0].className).toBe(
      "someClass icon icon-someIcon"
    );
    expect(rendered.refs.list.children[2].children[0].className).toBe(
      "anotherClass"
    );
    ...
    expect(element.suggestions[element.suggestionsIndex].title).toBe(
      rendered.refs.list.children[0].textContent
    );
    ...
    expect(element.suggestionsIndex).toBe(1);
    expect(element.suggestions[element.suggestionsIndex].title).toBe(
      rendered.refs.list.children[1].textContent
    );
    ...
    expect(element.suggestions[element.suggestionsIndex].title).toBe(
      rendered.refs.list.children[2].textContent
    );
